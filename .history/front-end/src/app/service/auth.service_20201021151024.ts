...
export class AuthService {
  ...
  updatePost(post: any): Observable<any> {
    const currentHeaders = this.myHeaders;
    currentHeaders.Authorization = localStorage.getItem('token');
    return this.http.put<any>('http://localhost:3000/account/post', post, { headers: currentHeaders })
      .pipe(
        map(res => res)
      );
  }
  ...
  deletePost(id): Observable<any> {
    const currentHeaders = this.myHeaders;
    currentHeaders.Authorization = localStorage.getItem('token');
    return this.http.delete<any>(`http://localhost:3000/post/${id}`, { headers: currentHeaders })
      .pipe(
        map(res => res)
      );
  }

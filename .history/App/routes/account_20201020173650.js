...
router.post('/post', async (req, res, next) => {
    try {
        ...
        await Post.addPost(newPost, (err, result) => {
            if (err) {
...
router.put('/post', async (req, res, next) => {
    try {
        ...
        await Post.updatePost(newPost, (err, result) => {
            if (err) {
                console.log('Account add Post:', err)
                res.json({ success: false, msg: 'Post has not been edited' + err });
            } else {
                res.json({ success: true, msg: 'Post has been edited' })
            }
        })
    } catch (err) {
        res.json({ success: false, msg: 'Crash edit Post:' + err });
    }
})
